...
public class LevelFiveLayer extends CCLayer
{
	...
	public LevelFiveLayer () 
	{
		...
		for (int i = 0; i < numberOfZombies; i++)
		{
			...
			float newSpawnTime = Math.abs(previousSpawnTime + (MathLib.random(1,1)));
		...
		CCSprite background = CCSprite.sprite("final_background.jpg");
		...
		CCSprite level = CCSprite.sprite("Final-Level.png");
	...
	{
		CCMoveTo actionMove = CCMoveTo.action(2, CGPoint.ccp(zombieSprite.getPosition().x, zombieSprite.getBoundingBox().size.height - 70 ));
	...
	public void checkHearts(float dt)
	{
		...
		{
			CCScene scene = GameWinLayer.scene(); //
	...
	public boolean ccTouchesBegan(MotionEvent event){
		...
		if(CGRect.containsPoint((_targets.get(1).getBoundingBox()), location)){
			...
			CCDirector.sharedDirector().pushScene(LevelFiveLayer.scene());
			CCDirector.sharedDirector().replaceScene(scene);
		}
		else{
	...
	public static CCScene scene()
	{
		...
		CCLayer layer = new LevelFiveLayer();
