...
public class LevelThreeLayer extends CCLayer
{
	...
	public LevelThreeLayer () 
	{
		p = new Player();
		numberOfZombies = 30; //more zombies
		...
		for (int i = 0; i < numberOfZombies; i++)
		{
			...
			float newSpawnTime = Math.abs(previousSpawnTime + (MathLib.random(1,3))); //faster spawn
		...
		CCSprite background = CCSprite.sprite("l3_background.jpg");
		...
		CCSprite level = CCSprite.sprite("Level-3.png");
	...
	{
		CCMoveTo actionMove = CCMoveTo.action(6, CGPoint.ccp(zombieSprite.getPosition().x, zombieSprite.getBoundingBox().size.height - 70 ));
	...
	public void checkHearts(float dt)
	{
		...
		{
			CCScene scene = TransitionLayer.scene(4); //
	...
	public boolean ccTouchesBegan(MotionEvent event){
		...
		if(CGRect.containsPoint((_targets.get(1).getBoundingBox()), location)){
			...
			CCDirector.sharedDirector().pushScene(LevelThreeLayer.scene());
			CCDirector.sharedDirector().replaceScene(scene);
		}
		else{
	...
	public static CCScene scene()
	{
		...
		CCLayer layer = new LevelThreeLayer();
