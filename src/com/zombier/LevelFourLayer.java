...
public class LevelFourLayer extends CCLayer
{
	...
	public LevelFourLayer () 
	{
		...
		for (int i = 0; i < numberOfZombies; i++)
		{
			...
			float newSpawnTime = Math.abs(previousSpawnTime + (MathLib.random(1,2)));
		...
		CCSprite background = CCSprite.sprite("l4_background.png");
		...
		CCSprite level = CCSprite.sprite("Level-4.png");
	...
	{
		CCMoveTo actionMove = CCMoveTo.action(4, CGPoint.ccp(zombieSprite.getPosition().x, zombieSprite.getBoundingBox().size.height - 70 ));
	...
	public void checkHearts(float dt)
	{
		...
		{
			CCScene scene = TransitionLayer.scene(5); //
	...
	public boolean ccTouchesBegan(MotionEvent event){
		...
		if(CGRect.containsPoint((_targets.get(1).getBoundingBox()), location)){
			...
			CCDirector.sharedDirector().pushScene(LevelFourLayer.scene());
			CCDirector.sharedDirector().replaceScene(scene);
		}
		else{
	...
	public static CCScene scene()
	{
		...
		CCLayer layer = new LevelFourLayer();
