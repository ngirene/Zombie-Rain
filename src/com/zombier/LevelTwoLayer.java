...
public class LevelTwoLayer extends CCLayer
{
	...
	public LevelTwoLayer () 
	{
		p = new Player();
		numberOfZombies = 20; //more zombies
		...
		for (int i = 0; i < numberOfZombies; i++)
		{
			...
			float newSpawnTime = Math.abs(previousSpawnTime + (MathLib.random(1,4))); //faster spawn
		...
		CCSprite background = CCSprite.sprite("l2_background.jpg");
		...
		CCSprite level = CCSprite.sprite("Level-2.png");
	...
	{
		CCMoveTo actionMove = CCMoveTo.action(8, CGPoint.ccp(zombieSprite.getPosition().x, zombieSprite.getBoundingBox().size.height - 70 )); //fall faster
	...
	public void checkHearts(float dt)
	{
		...
		{
			CCScene scene = TransitionLayer.scene(3); //
	...
	public boolean ccTouchesBegan(MotionEvent event){
		...
		if(CGRect.containsPoint((_targets.get(1).getBoundingBox()), location)){
			...
			CCDirector.sharedDirector().pushScene(LevelTwoLayer.scene());
			CCDirector.sharedDirector().replaceScene(scene);
		}
		else{
	...
	public static CCScene scene()
	{
		...
		CCLayer layer = new LevelTwoLayer();
